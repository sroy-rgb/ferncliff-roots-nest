import { r as reactExports, T as jsxRuntimeExports } from "./server-DxfjOg74.js";
import { p, L as Link } from "./router-C6JYablr.js";
import { n, s } from "./TwitterLogo.es-CSs4YNFO.js";
import { u as useScrollReveal, N as Nav, F as Footer, M as MobileBottomBar } from "./MobileBottomBar-BE_2in2-.js";
const e$1 = /* @__PURE__ */ new Map([
  [
    "bold",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement("path", { d: "M117.18,188.74a12,12,0,0,1,0,17l-5.12,5.12A58.26,58.26,0,0,1,70.6,228h0A58.62,58.62,0,0,1,29.14,127.92L63.89,93.17a58.64,58.64,0,0,1,98.56,28.11,12,12,0,1,1-23.37,5.44,34.65,34.65,0,0,0-58.22-16.58L46.11,144.89A34.62,34.62,0,0,0,70.57,204h0a34.41,34.41,0,0,0,24.49-10.14l5.11-5.12A12,12,0,0,1,117.18,188.74ZM226.83,45.17a58.65,58.65,0,0,0-82.93,0l-5.11,5.11a12,12,0,0,0,17,17l5.12-5.12a34.63,34.63,0,1,1,49,49L175.1,145.86A34.39,34.39,0,0,1,150.61,156h0a34.63,34.63,0,0,1-33.69-26.72,12,12,0,0,0-23.38,5.44A58.64,58.64,0,0,0,150.56,180h.05a58.28,58.28,0,0,0,41.47-17.17l34.75-34.75a58.62,58.62,0,0,0,0-82.91Z" }))
  ],
  [
    "duotone",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement(
      "path",
      {
        d: "M218.34,119.6,183.6,154.34a46.58,46.58,0,0,1-44.31,12.26c-.31.34-.62.67-.95,1L103.6,202.34A46.63,46.63,0,1,1,37.66,136.4L72.4,101.66A46.6,46.6,0,0,1,116.71,89.4c.31-.34.62-.67,1-1L152.4,53.66a46.63,46.63,0,0,1,65.94,65.94Z",
        opacity: "0.2"
      }
    ), /* @__PURE__ */ reactExports.createElement("path", { d: "M240,88.23a54.43,54.43,0,0,1-16,37L189.25,160a54.27,54.27,0,0,1-38.63,16h-.05A54.63,54.63,0,0,1,96,119.84a8,8,0,0,1,16,.45A38.62,38.62,0,0,0,150.58,160h0a38.39,38.39,0,0,0,27.31-11.31l34.75-34.75a38.63,38.63,0,0,0-54.63-54.63l-11,11A8,8,0,0,1,135.7,59l11-11A54.65,54.65,0,0,1,224,48,54.86,54.86,0,0,1,240,88.23ZM109,185.66l-11,11A38.41,38.41,0,0,1,70.6,208h0a38.63,38.63,0,0,1-27.29-65.94L78,107.31A38.63,38.63,0,0,1,144,135.71a8,8,0,0,0,7.78,8.22H152a8,8,0,0,0,8-7.78A54.86,54.86,0,0,0,144,96a54.65,54.65,0,0,0-77.27,0L32,130.75A54.62,54.62,0,0,0,70.56,224h0a54.28,54.28,0,0,0,38.64-16l11-11A8,8,0,0,0,109,185.66Z" }))
  ],
  [
    "fill",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement("path", { d: "M208,32H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM115.7,192.49a43.31,43.31,0,0,1-55-66.43l25.37-25.37a43.35,43.35,0,0,1,61.25,0,42.9,42.9,0,0,1,9.95,15.43,8,8,0,1,1-15,5.6A27.33,27.33,0,0,0,97.37,112L72,137.37a27.32,27.32,0,0,0,34.68,41.91,8,8,0,1,1,9,13.21Zm79.61-62.55-25.37,25.37A43,43,0,0,1,139.32,168h0a43.35,43.35,0,0,1-40.53-28.12,8,8,0,1,1,15-5.6A27.35,27.35,0,0,0,139.28,152h0a27.14,27.14,0,0,0,19.32-8L184,118.63a27.32,27.32,0,0,0-34.68-41.91,8,8,0,1,1-9-13.21,43.32,43.32,0,0,1,55,66.43Z" }))
  ],
  [
    "light",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement("path", { d: "M238,88.18a52.42,52.42,0,0,1-15.4,35.66l-34.75,34.75A52.28,52.28,0,0,1,150.62,174h-.05A52.63,52.63,0,0,1,98,119.9a6,6,0,0,1,6-5.84h.17a6,6,0,0,1,5.83,6.16A40.62,40.62,0,0,0,150.58,162h0a40.4,40.4,0,0,0,28.73-11.9l34.75-34.74A40.63,40.63,0,0,0,156.63,57.9l-11,11a6,6,0,0,1-8.49-8.49l11-11a52.62,52.62,0,0,1,74.43,0A52.83,52.83,0,0,1,238,88.18Zm-127.62,98.9-11,11A40.36,40.36,0,0,1,70.6,210h0a40.63,40.63,0,0,1-28.7-69.36L76.62,105.9A40.63,40.63,0,0,1,146,135.77a6,6,0,0,0,5.83,6.16H152a6,6,0,0,0,6-5.84A52.63,52.63,0,0,0,68.14,97.42L33.38,132.16A52.63,52.63,0,0,0,70.56,222h0a52.26,52.26,0,0,0,37.22-15.42l11-11a6,6,0,1,0-8.49-8.48Z" }))
  ],
  [
    "regular",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement("path", { d: "M240,88.23a54.43,54.43,0,0,1-16,37L189.25,160a54.27,54.27,0,0,1-38.63,16h-.05A54.63,54.63,0,0,1,96,119.84a8,8,0,0,1,16,.45A38.62,38.62,0,0,0,150.58,160h0a38.39,38.39,0,0,0,27.31-11.31l34.75-34.75a38.63,38.63,0,0,0-54.63-54.63l-11,11A8,8,0,0,1,135.7,59l11-11A54.65,54.65,0,0,1,224,48,54.86,54.86,0,0,1,240,88.23ZM109,185.66l-11,11A38.41,38.41,0,0,1,70.6,208h0a38.63,38.63,0,0,1-27.29-65.94L78,107.31A38.63,38.63,0,0,1,144,135.71a8,8,0,0,0,16,.45A54.86,54.86,0,0,0,144,96a54.65,54.65,0,0,0-77.27,0L32,130.75A54.62,54.62,0,0,0,70.56,224h0a54.28,54.28,0,0,0,38.64-16l11-11A8,8,0,0,0,109,185.66Z" }))
  ],
  [
    "thin",
    /* @__PURE__ */ reactExports.createElement(reactExports.Fragment, null, /* @__PURE__ */ reactExports.createElement("path", { d: "M236,88.12a50.44,50.44,0,0,1-14.81,34.31l-34.75,34.74A50.33,50.33,0,0,1,150.62,172h-.05A50.63,50.63,0,0,1,100,120a4,4,0,0,1,4-3.89h.11a4,4,0,0,1,3.89,4.11A42.64,42.64,0,0,0,150.58,164h0a42.32,42.32,0,0,0,30.14-12.49l34.75-34.74a42.63,42.63,0,1,0-60.29-60.28l-11,11a4,4,0,0,1-5.66-5.65l11-11A50.64,50.64,0,0,1,236,88.12ZM111.78,188.49l-11,11A42.33,42.33,0,0,1,70.6,212h0a42.63,42.63,0,0,1-30.11-72.77l34.75-34.74A42.63,42.63,0,0,1,148,135.82a4,4,0,0,0,8,.23A50.64,50.64,0,0,0,69.55,98.83L34.8,133.57A50.63,50.63,0,0,0,70.56,220h0a50.33,50.33,0,0,0,35.81-14.83l11-11a4,4,0,1,0-5.65-5.66Z" }))
  ]
]);
const e = reactExports.forwardRef((r, t) => /* @__PURE__ */ reactExports.createElement(p, { ref: t, ...r, weights: e$1 }));
e.displayName = "LinkIcon";
const c = e;
function StoryPage({
  title,
  heroImage,
  category,
  date,
  readTime,
  authorName,
  authorTitle,
  authorInitials,
  sections,
  children,
  relatedStories = []
}) {
  useScrollReveal();
  const articleRef = reactExports.useRef(null);
  const [barWidth, setBarWidth] = reactExports.useState(0);
  const [activeIdx, setActiveIdx] = reactExports.useState(0);
  const [sidebarVisible, setSidebarVisible] = reactExports.useState(false);
  reactExports.useEffect(() => {
    const onScroll = () => {
      const el = articleRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const total = el.offsetHeight;
      const scrolled = Math.max(0, Math.min(total, -rect.top));
      setBarWidth(Math.min(100, scrolled / total * 100));
      setSidebarVisible(rect.top < window.innerHeight * 0.4 && rect.bottom > window.innerHeight * 0.3);
      let idx = 0;
      sections.forEach((s2, i) => {
        const sec = document.getElementById(s2.id);
        if (sec && sec.getBoundingClientRect().top < window.innerHeight * 0.4) idx = i;
      });
      setActiveIdx(idx);
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    onScroll();
    return () => window.removeEventListener("scroll", onScroll);
  }, [sections]);
  const scrollTo = (id) => {
    document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-offwhite", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Nav, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "reading-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "reading-bar-fill", style: { width: `${barWidth}%` } }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `reading-progress ${sidebarVisible ? "visible" : ""}`, children: sections.map((s2, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        onClick: () => scrollTo(s2.id),
        className: `rp-item ${i === activeIdx ? "active" : ""} ${i < activeIdx ? "passed" : ""}`,
        type: "button",
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rp-dot-wrap", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rp-dot" }),
            i < sections.length - 1 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rp-line" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rp-label", children: s2.label })
        ]
      },
      s2.id
    )) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "article-hero", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: heroImage, alt: title }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "article-hero-content", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "container-n", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "breadcrumb", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/", children: "Home" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "›" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/stories", children: "Stories of Impact" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "›" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "article-meta", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "article-tag", children: category }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "article-date", children: [
            date,
            " · ",
            readTime,
            " read"
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: title }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "article-author", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "article-author-avatar", children: authorInitials }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "article-author-info", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: authorName }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: authorTitle })
          ] })
        ] })
      ] }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { ref: articleRef, className: "article-body reveal", children: [
      children,
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "share-bar", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Share" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "#", className: "share-btn", "aria-label": "Share on Facebook", children: /* @__PURE__ */ jsxRuntimeExports.jsx(n, { size: 18 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "#", className: "share-btn", "aria-label": "Share on Twitter", children: /* @__PURE__ */ jsxRuntimeExports.jsx(s, { size: 18 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "#", className: "share-btn", "aria-label": "Copy link", children: /* @__PURE__ */ jsxRuntimeExports.jsx(c, { size: 18 }) })
      ] })
    ] }),
    relatedStories.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "py-20 bg-light", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "text-center mb-12", style: { fontSize: 32 }, children: [
        "More Stories of ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hw", children: "Impact" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-6 max-w-[1200px] mx-auto px-8", style: { gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }, children: relatedStories.map((r, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        Link,
        {
          to: r.href,
          className: "block bg-white rounded-[20px] overflow-hidden group transition-all duration-[400ms] hover:-translate-y-1",
          style: { boxShadow: "0 2px 16px rgba(44,41,38,0.06)" },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-[200px] overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: r.image, alt: r.title, className: "w-full h-full object-cover transition-transform duration-[600ms] group-hover:scale-[1.04]" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-6", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(RelTag, { variant: r.tagVariant ?? "teal", children: r.category }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-sans text-[16px] font-bold text-dark mt-2.5 mb-1.5 leading-snug", children: r.title }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[13px] text-text-muted", children: r.excerpt })
            ] })
          ]
        },
        i
      )) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Footer, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx(MobileBottomBar, {})
  ] });
}
function RelTag({ variant, children }) {
  const styles = {
    teal: { background: "var(--color-teal-light)", color: "var(--color-teal)" },
    coral: { background: "var(--color-coral-light)", color: "var(--color-coral)" },
    gold: { background: "var(--color-gold-light)", color: "#96751e" }
  }[variant];
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "inline-block px-3 py-[3px] rounded-full text-[11px] font-bold", style: styles, children });
}
function Anchor({ id }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id });
}
function PullQuote({ children, centered = false }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `pull-quote ${centered ? "centered" : ""}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children }) });
}
function InlineQuote({ children }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "inline-quote", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children }) });
}
function FloatRight({ src, alt, caption }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "float-right", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src, alt }),
    caption && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "caption", children: caption })
  ] });
}
function ImageGrid({
  cols = 3,
  images,
  caption,
  portrait = false
}) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `img-grid cols-${cols}`, children: [
    images.map((im, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `img-grid-item ${portrait ? "portrait" : ""}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: im.src, alt: im.alt }) }, i)),
    caption && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "img-grid-caption", children: caption })
  ] });
}
function ArticleDivider() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "article-divider" });
}
function ArticleCTA({
  title,
  accent,
  description,
  buttonLabel = "Support Ferncliff",
  buttonHref = "/#giving"
}) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "article-cta", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { children: [
      title,
      " ",
      accent && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hw", children: accent })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: description }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: buttonHref, className: "btn btn-teal", children: buttonLabel })
  ] });
}
export {
  Anchor as A,
  FloatRight as F,
  InlineQuote as I,
  PullQuote as P,
  StoryPage as S,
  ImageGrid as a,
  ArticleDivider as b,
  ArticleCTA as c
};
